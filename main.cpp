...
int main(int argc, char *argv[])
{
    ...
    modbus_set_response_timeout(ctx, 0, 1 * 1000 / 2); //设置modbus超时等待。环境监控采集要求超时设置为300ms
    ...
    for (;;)
    {
        ...
        rc = modbus_read_registers(ctx, 0,
                                   read_nb, tab_rp_registers);
        LOG(INFO) << "222222222222222222222 == ";

        UINT16 tmpvalue = tab_rp_registers[0];
        LOG(INFO) << "tmpvalue == " << tmpvalue;
        LogInfoDump("tab_rp_registers = ", tab_rp_registers, read_nb);
        sleep(1);
    }
